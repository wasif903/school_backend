...
const HandleNewAdmission = async (req, reply) => {
  try {
    ...
    // Handle base64 image saving for parent picture
    let parentPicturePath;
    if (picture) {
      parentPicturePath = await SaveBase64Image(
        picture,
    ...
    const documentPathsForStudents = await Promise.all(
      students.map((student) =>
        student.Documents && student.Documents.length > 0
          ? Promise.all(
              student.Documents.map(async (document, index) => {
                // Save each document as a base64 image
                return SaveBase64Image(
                  document.fileUrl,
                  "documents",
                  `${uuidv4()}_document_${index}.jpg`
                );
              })
            )
          : []
    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      // Create the students and handle their documents
      const studentsData = await Promise.all(
        students.map(async (student, index) => {
          const { feeCards, Documents, ...studentData } = student;
          ...
          if (Documents && Documents.length > 0) {
            await Promise.all(
              Documents.map(async (document, docIndex) => {
                await prisma.document.create({
                  data: {
                    studentId: newStudent.id,
                    fileUrl: documentPathsForStudents[index][docIndex], // Use the specific document's file URL
                  },
                });
              })
            );
